(function () {
  class MilestoneOutstanding extends HTMLElement {
    ...
    async serializeCustomWidgetToImage() {
      ...
      /* Fixed footer area */
      const footerReservedHeight = 140;

      /* Use actual widget height */
      const canvasHeight =
      ...
      let startRowsY = headerHeight + 15;

      /* Calculate available area only for rows */
      ...
      /* Only draw rows that fit */
      const visibleRows = Math.max(1, Math.floor(rowsAreaHeight / rowHeight));
      ...
      ctx.fillText("INVOICED", width * 0.5, y + 25);
      ctx.fillText("OUTSTANDING", width * 0.68, y + 25);
      ...
      data.slice(0, visibleRows).forEach((item) => {
        ...
        ctx.fillText(milestoneText, 25, y + 22);
        ...
        ctx.fillText(this.formatRowValue(item.invoiced), width * 0.5, y + 22);
        ...
        ctx.beginPath();
        ctx.roundRect(barX, y + 14, 80, 8, 4);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.roundRect(barX, y + 14, (80 * percent) / 100, 8, 4);
        ctx.fill();
      ...
      if (totalRows > visibleRows) {
        const trackHeight = rowsAreaHeight;

        const trackY = headerHeight + 60;

        const trackX = width - 24;
        ...
        const thumbHeight = Math.min(
          trackHeight,
          Math.max(30, (visibleRows / totalRows) * trackHeight),
        );
      ...
      const footerY = canvasHeight - footerReservedHeight;
